...
import { IconPackage, IconCart, IconMap, IconStar, IconOrders } from '@/components/ui/Icons';
...
export default function HomePage() {
  ...
  useEffect(() => {
    apiClient.getProducts({ page: '1', limit: '20' }).then((r: any) => {
      const all = r.data || [];
    ...
    <div className="space-y-8 md:space-y-14">
      ...
      >
        ...
        <div className="relative z-10">
          ...
          >
            <Link href="/products" className="bg-white text-blue-600 px-5 md:px-8 py-2.5 md:py-3 rounded-full text-sm md:text-base font-semibold hover:bg-blue-50 transition shadow-lg">
              Voir les produits
            </Link>
            {!isAuthenticated && (
              <Link href="/register" className="border-2 border-white text-white px-5 md:px-8 py-2.5 md:py-3 rounded-full text-sm md:text-base font-semibold hover:bg-white/10 transition">
                Créer un compte
              </Link>
            )}
          </motion.div>
      ...
      {newArrivals.length > 0 && (
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-50px" }} variants={fadeInUp} transition={{ duration: 0.5 }}>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-5">
            {newArrivals.map((p, i) => {
              const img = getImageUrl(p.image_url || p.thumbnail);
              return (
                <motion.div key={p.id} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }}
                  ...
                  <div className="relative">
                    ...
                    <Link href={`/products/${p.id}`} className="block h-36 md:h-48 bg-gray-50 flex items-center justify-center overflow-hidden">
                      {img ? <img src={img} alt={p.name} className="w-full h-full object-cover group-hover:scale-105 transition duration-300" /> : <IconPackage size={40} className="text-gray-300" />}
                    </Link>
                  </div>
                  <div className="p-3 md:p-4">
                    ...
                    <div className="flex items-center gap-2 mt-1.5">
                      ...
                      {p.compare_price && <span className="text-xs text-gray-400 line-through">{p.compare_price?.toFixed(2)} €</span>}
                    </div>
                    {isAuthenticated && (
                      <button onClick={() => handleAddToCart(p.id, p.name)} className="w-full mt-2 bg-blue-600 text-white py-1.5 rounded-lg text-xs md:text-sm hover:bg-blue-700 transition flex items-center justify-center gap-1">
      ...
      {popularProducts.length > 0 && (
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-50px" }} variants={fadeInUp} transition={{ duration: 0.5 }}>
          <div className="flex justify-between items-center mb-4 md:mb-6">
            <div>
              <h2 className="text-xl md:text-2xl font-bold flex items-center gap-2"><IconStar size={20} className="text-yellow-500" />Produits populaires</h2>
      ...
      <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-50px" }} variants={fadeInUp} transition={{ duration: 0.5 }}
        ...
        <p className="text-gray-300 text-sm md:text-base mb-5 md:mb-6 max-w-lg mx-auto">Notre équipe de techniciens est disponible pour vous assister.</p>
