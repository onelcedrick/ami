...
func init() {
	// Chercher la clé publique à plusieurs endroits
		...
		if err == nil {
			...
			if block != nil {
				...
				if err == nil {
					...
					log.Println("✅ Cart Service: Clé JWT chargée depuis", p)
					return
				}
			}
		}
	}
	log.Println("⚠️ Cart Service: Pas de clé JWT trouvée, utilisation X-User-ID")
}

func AuthMiddleware(c *fiber.Ctx) error {
	// 1. Essayer JWT d'abord
	...
	// 2. Fallback X-User-ID (développement)
